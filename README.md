...
# Pattern-Guided Arithmetic Optimizations with MLIR & TinyTapeout (`ttihp04`, IEEE754 per-op variant)
...
In this repository variant, the floating-point lowering path is `per-op` (IEEE754 per operation), not the specialized S3FDP accumulator mode.
It targets open-source silicon through TinyTapeout on IHP.
...
- arithmetic IR work around multi-level compilation, real-number expressions, fixed-point or customized accumulation, and compiler-visible numeric choices
...
- `emeraude-mlir` (my current repository, not yet open) for lowering and arithmetic-mode control
...
## Per-op IEEE754 lowering and comb stage

Lowering mode is configured in `scripts/generate_s3fdp_core.sh`:

- `lowering-mode=per-op`
...
Here CIRCT serves as the structural backend, while the floating-point path is made explicit through `flopoco-arith-to-comb` in per-op IEEE754 mode, the surrounding arithmetic-lowering work in `emeraude-mlir`, and the FloPoCo2 IR-generation refactoring above.
